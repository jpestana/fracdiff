function [t,it,rv] = Ex2_Const_Nu_Diff_Run

% [t,it,rv] = Ex2_Const_Nu_Diff_Run
%
% Example 2, Table 5.4 from J. Pestana, Preconditioners for symmetrized 
...
% Fractional diffusion order
alpha = 1.5;

gvec = (2.^(12:2:18))-1; % Problem dimensions
...
% Store results
t(n_grid,1:n_type,n_diff) = 0;
it(n_grid,n_time,1:n_type,n_diff) = 0;
rv(n_grid,1:n_type,n_diff) = 0;
runt(n_grid,1) = 0;
...
for gr = 1:n_grid
    N = gvec(gr);
    lev = log2(N+1);
    stoplev = lev-7; % lNumber of levels for multigrid
    ...
    for k = 1:n_diff
        ...
        for ts = 1:mtstep
            b = Fdx(:,ts) + nu*u_old;
            [u_old,~,itgp] = rpgmres(@(x)mtimes(Asmtmult,x),b,tol,20,1,@(x)mycircinv(invlam,x),x0);
            U1(:,ts) = u_old;
            it(gr,ts,1,k) = itgp(2);
        end
        t(gr,1,k) = toc;
        ...
        for ts = 1:mtstep
            b = Fdx(:,ts) + nu*u_old;
            [u_old,~,~,itls] = lsqr(@(x,transp_flag)qmrmtimes(Asmtmult,x,transp_flag),b,tol,N,@(x,transp_flag)qmrmldivide(invlam,conj(invlam),x,transp_flag),[],x0);
            U3(:,ts) = u_old;
            it(gr,ts,3,k) = itls;
        end
        t(gr,3,k) = toc;
        ...
        for ts = 1:mtstep
            ...
            [u_old,~,~,itmr] = minres(@(x)ymtimes(Asmtmult,x),yb,tol,N,@(x)mycircinv(invlam,x),[],x0);
            U5(:,ts) = u_old;
            it(gr,ts,5,k) = itmr;
        end
        t(gr,5,k) = toc;
      ...
      % MINRES with MG AF
        ...
        save Ex2_Const_Nu_Diff t it rv runt gvec d1 d2 L R T dlist mtstep prob
        
    end
end

% Make table
fid = fopen('Ex2_Const_Nu_Diff.txt','w+');
for k=1:n_diff
    fprintf(fid,'\\hline\n');
    fprintf(fid,'\\multirow{%i}{*}{(%g,%g)}',n_grid,dlist(k,:));
    for j = 1:n_grid
...
if strcmp(transp_flag,'transp')
    y = vcycle_fastmv_nr(diageltrans,Asmtrans,Actrans,y,2,2,stoplev,0.7);
else
    y = vcycle_fastmv_nr(diagel,Asm,Ac,y,2,2,stoplev,0.7);
end
end

% Matrix-vector product with flipped Toeplitz matirx
